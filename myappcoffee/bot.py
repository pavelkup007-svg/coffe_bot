# import logging
# import threading
# from myappcoffee.views import start_bot  # Импорт функции запуска бота
#
...
ADMIN_USER_IDS = [740586983, 372042591, 211600094]  # Добавлен второй администратор
# Хранилище состояния для проверки пароля
user_sessions = {}
...
def start_message(message):
    ...
    logging.info(f"Received /start command from user_id: {user_id}, {user.first_name}")
...
@bot.message_handler(func=lambda message: message.text == 'Отчет' and message.from_user.id in [740586983, 372042591, 211600094])
...
bot.polling(
    ...
    timeout=30,      # Стандартное ожидание
    interval=5       # Увеличенная пауза из-за проблем с прокси
)
